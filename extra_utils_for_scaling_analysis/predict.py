        ...
        if motion is not None:
            ...
            for l in tqdm(range(len(lams))):  # try various regularization
                ...
                for k in tqdm(range(len(nrank1))):  # try various ranks
                    ...
                    window_length = 51 
                    polyorder = 11       
                    vp_train = savgol_filter(vp_train.T, window_length=window_length, polyorder=polyorder, axis=0).T
                    vp_test = savgol_filter(vp_test.T, window_length=window_length, polyorder=polyorder, axis=0).T

                    def center_data(data, axis=0):
                        mean = np.mean(data, axis=axis, keepdims=True)
                        centered_data = data - mean
                        return centered_data
                    vp_train=center_data(vp_train,axis=1)
                    vp_test =center_data(vp_test, axis=1)

                    compute_500_4000=1
                    if compute_500_4000:
                        s1 = projs1[itest[(itest >= 500) & (itest <= 4000)], :].T - vp_train[:, itest[(itest >= 500) & (itest <= 4000)]]
                        s2 = projs2[itest[(itest >= 500) & (itest <= 4000)], :].T - vp_test[:, itest[(itest >= 500) & (itest <= 4000)]]
                    else:
                        s1 = projs1[itest[(itest <= 500) | (itest >= 4000)], :].T - vp_train[:, itest[(itest <= 500) | (itest >= 4000)]]
                        s2 = projs2[itest[(itest <= 500) | (itest >= 4000)], :].T - vp_test[:, itest[(itest <= 500)  | (itest >= 4000)]]
