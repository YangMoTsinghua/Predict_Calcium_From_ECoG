...
def run_SVCA_partition(
    ...
    **kwargs
    ...
    print("ntrain:", ntrain.shape,min(ntrain),"-",max(ntrain))
    print("ntest:", ntest.shape,min(ntest),"-",max(ntest))
    ...
    print("interleave i:", interleave)
    print("itrain:", itrain.shape, min(itrain), "-", max(itrain))
    print("itest:", itest.shape, min(itest), "-", max(itest))
    ### RUN SVCA
